"""ReliVE-v2 Stage 3G-A v3.1 strict JSON-object anchor grounding."""
...
FORMAT="relive-v2-spatial-anchor-grounding-v3.1"
...
def _prompt(entry:dict[str,Any])->str:
 ...
 text=files('relive').joinpath('prompts','v2_tal_spatial_anchor_grounding_v3_1.txt').read_text(encoding='utf-8').format(observation_claim=entry['observation_claim'],observation_role=entry['observation_role'],required_roles=', '.join(required),contextual_roles=', '.join(context) or 'none',role_descriptions='\n'.join(f'- {r}: {ROLES[r]}' for r in all_roles)).rstrip()
 return text

def _parse(raw:str,expected:list[str],width:int,height:int)->tuple[list[dict[str,Any]],str|None]:
 text=raw.strip()
 # v3_1's generated object contract does not permit Markdown fences or arrays.
...
def preflight(*,stage3f_dir:Path,stage3c_dir:Path,stage3d_dir:Path,stage3e_dir:Path,video_index_dir:Path,config_path:Path,policy_path:Path,output_dir:Path,smoke_selection_manifest:Path|None=None,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
 ...
 if (policy.get('format')!='relive-v2-spatial-anchor-grounding-v3.1-policy-v1'
     or policy.get('json_schema_constrained_decoding_supported') is not False
     or policy.get('structured_generation_enforcement')!='PROMPT_ONLY_FAIL_CLOSED'):raise SpatialAnchorGroundingError('POLICY_INVALID')
 ...
 out={'format':FORMAT,'status':'PASS','mode':'preflight','provider_type':policy['provider_type'],'provider_name':policy['provider_name'],'stage3f_manifest_sha256':upstream['stage3f_manifest_sha256'],'upstream_sha256':upstream,'policy_sha256':sha(policy_path),'config_sha256':sha(config_path),'generation_parameters':effective['backend']['generation'],'generation_contract':{'generation_contract_version':policy['generation_contract_version'],'native_generation_metadata_supported':True,'json_schema_constrained_decoding_supported':False,'structured_generation_enforcement':policy['structured_generation_enforcement'],'enforcement':'VERSIONED_COMPLETE_EXAMPLES_AND_FAIL_CLOSED_PARSER'},'model_fingerprint':fingerprint,'planned_model_calls':len(entries),'unique_task_anchor_count':len(entries),'frozen_calls':frozen,**selection,'model_calls_made':0,'new_model_calls':0,'cache_hits':0,'backend_loaded':True,'cache_opened':False,'frames_read':len(entries),'unique_frame_bytes_opened':len({e['frame_sha256'] for e in entries}),'videos_read':0,'gt_used':False,'assistant_or_gt_values_accessed':False,'boxes_created':0,'points_created':0,'masks_created':0,'masklets_created':0,'support_tubes_created':0,'certificate_created':False,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE'}
 out['preflight_content_sha256']=stable_hash(out);output_dir.mkdir(parents=True);write(output_dir/'stage3g_v3_1_preflight.json',out);return out

def _preflight(root:Path)->dict[str,Any]:
 p=obj(root/'stage3g_v3_1_preflight.json','PREFLIGHT')
...
def execute(*,output_dir:Path,config_path:Path,policy_path:Path,mode:str,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
 ...
 for entry in entries:
  ...
  try: stored=cache.get_json('stage3g_v3_1',key)
  ...
  if stored is not None:
   ...
   cache.put_json('stage3g_v3_1',key,{'cache_key':key,'result':result});new+=1;cache_hit=False
 ...
 for name,value in {'v2_tal_spatial_anchor_groundings_v3_1.jsonl':results,'v2_tal_spatial_anchor_grounding_v3_1_unresolved.jsonl':unresolved,'v2_tal_spatial_anchor_grounding_v3_1_bindings.jsonl':bindings,'v2_tal_spatial_anchor_grounding_v3_1_trace.jsonl':traces}.items():write(run_dir/name,value)
 review=[_review(run_dir,row) for row in results];write(run_dir/'v2_tal_spatial_anchor_v3_1_review_packet_index.jsonl',review)
 hashes={path.name:sha(path) for path in run_dir.glob('v2_tal_spatial_anchor_grounding*.jsonl')}|{'v2_tal_spatial_anchor_v3_1_review_packet_index.jsonl':sha(run_dir/'v2_tal_spatial_anchor_v3_1_review_packet_index.jsonl')}
...
def validate(*,output_dir:Path,mode:str='run')->dict[str,Any]:
 ...
 result=rows(run/'v2_tal_spatial_anchor_groundings_v3_1.jsonl','GROUNDINGS')
 ...
 review=rows(run/'v2_tal_spatial_anchor_v3_1_review_packet_index.jsonl','REVIEW_PACKET_INDEX')
