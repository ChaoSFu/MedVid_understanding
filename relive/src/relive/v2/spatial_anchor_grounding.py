...
FORMAT="relive-v2-spatial-anchor-grounding-v1"
...
def _prompt(entry:dict[str,Any])->str:
 ...
 text=files('relive').joinpath('prompts','v2_tal_spatial_anchor_grounding.txt').read_text(encoding='utf-8').format(observation_claim=entry['observation_claim'],observation_role=entry['observation_role'],required_roles=', '.join(required),contextual_roles=', '.join(context) or 'none',role_descriptions='\n'.join(f'- {r}: {ROLES[r]}' for r in all_roles)).rstrip()
 return text
...
def preflight(*,stage3f_dir:Path,stage3c_dir:Path,stage3d_dir:Path,stage3e_dir:Path,video_index_dir:Path,config_path:Path,policy_path:Path,output_dir:Path,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
...
def _preflight(root:Path)->dict[str,Any]:
 p=obj(root/'stage3g_preflight.json','PREFLIGHT')
...
def execute(*,output_dir:Path,config_path:Path,policy_path:Path,mode:str,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
 ...
 for entry in entries:
  ...
  try: stored=cache.get_json('stage3g',key)
  ...
  if stored is not None:
   ...
   raw=backend.infer({'prompt':entry['prompt'],'image_paths':[entry['image_path']],'frame_ids':[entry['unique_visual_frame_id']]})
   ...
   result={'raw_response':raw,'raw_response_sha256':hashlib.sha256(raw.encode()).hexdigest(),'components':components,'failure_reason':failure,'status':_status(components,entry['required_component_roles'],failure)}
   cache.put_json('stage3g',key,{'cache_key':key,'result':result});new+=1;cache_hit=False
  row={k:v for k,v in entry.items() if k not in {'prompt','image_path_sha256'}}|result|{'provider_type':policy['provider_type'],'provider_name':policy['provider_name'],'parser_version':policy['parser_version'],'coordinate_conversion_policy_version':policy['coordinate_conversion_policy_version'],'stage3f_manifest_sha256':pre['stage3f_manifest_sha256'],'cache_key':key,'cache_hit':cache_hit}
  row['canonical_result_sha256']=stable_hash({k:v for k,v in row.items() if k not in {'cache_key','cache_hit','raw_response'}})
  results.append(row);traces.append({'spatial_grounding_task_id':row['spatial_grounding_task_id'],'anchor_candidate_id':row['anchor_candidate_id'],'cache_key':key,'cache_hit':cache_hit,'status':row['status'],'failure_reason':row['failure_reason'],'raw_response_sha256':row['raw_response_sha256']})
 ...
 for name,value in {'v2_tal_spatial_anchor_groundings.jsonl':results,'v2_tal_spatial_anchor_grounding_unresolved.jsonl':unresolved,'v2_tal_spatial_anchor_grounding_bindings.jsonl':bindings,'v2_tal_spatial_anchor_grounding_trace.jsonl':traces}.items():write(run_dir/name,value)
 review=[_review(run_dir,row) for row in results];write(run_dir/'v2_tal_spatial_anchor_review_packet_index.jsonl',review)
 hashes={path.name:sha(path) for path in run_dir.glob('v2_tal_spatial_anchor_grounding*.jsonl')}|{'v2_tal_spatial_anchor_review_packet_index.jsonl':sha(run_dir/'v2_tal_spatial_anchor_review_packet_index.jsonl')}
 ...
 manifest={'format':FORMAT,'status':'PASS','mode':mode,'stage_status':'SPATIAL_ANCHOR_GROUNDINGS_GENERATED_UNVERIFIED','candidate_status':CANDIDATE,'stage3f_manifest_sha256':pre['stage3f_manifest_sha256'],'upstream_sha256':pre['upstream_sha256'],'preflight_sha256':sha(output_dir/'stage3g_preflight.json'),'provider_type':policy['provider_type'],'provider_name':policy['provider_name'],'model_fingerprint':pre['model_fingerprint'],'policy_sha256':pre['policy_sha256'],'config_sha256':pre['config_sha256'],'planned_model_calls':expected,'new_model_calls':new,'cache_hits':hits,'logical_model_calls':expected,'canonical_grounding_result_sha256':stable_hash(canonical),'artifact_sha256':hashes,'grounding_result_count':len(results),'unresolved_count':len(unresolved),'review_item_count':len(review),'frames_read':expected,'unique_frame_bytes_opened':len({r['frame_sha256'] for r in results}),'videos_read':0,'model_calls_made':new,'backend_loaded':True,'cache_opened':True,'gt_used':False,'assistant_or_gt_values_accessed':False,'boxes_created':sum(1 for r in results for c in r['components'] if c['bbox_2d_raw'] is not None),'points_created':0,'masks_created':0,'masklets_created':0,'support_tubes_created':0,'certificate_created':False,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE','ready_for_spatial_grounding_audit':bool(results),'ready_for_dense_box_tube_generation':False,'ready_for_sam2_propagation':False,'ready_for_evidence_verification':False}
 manifest['manifest_content_sha256']=stable_hash(manifest);write(run_dir/'v2_tal_stage3g_manifest.json',manifest)
 audit={k:manifest[k] for k in ('format','status','mode','stage3f_manifest_sha256','upstream_sha256','planned_model_calls','new_model_calls','cache_hits','logical_model_calls','frames_read','unique_frame_bytes_opened','videos_read','model_calls_made','backend_loaded','cache_opened','gt_used','assistant_or_gt_values_accessed','boxes_created','points_created','masks_created','masklets_created','support_tubes_created','certificate_created','new_verified_count','certificate_status')}|{'artifact_sha256':{**hashes,'v2_tal_stage3g_manifest.json':sha(run_dir/'v2_tal_stage3g_manifest.json')}}
 audit['audit_content_sha256']=stable_hash(audit);write(run_dir/'v2_tal_stage3g_audit.json',audit)
 summary={k:manifest[k] for k in ('format','status','mode','stage_status','planned_model_calls','new_model_calls','cache_hits','logical_model_calls','canonical_grounding_result_sha256','grounding_result_count','unresolved_count','review_item_count','model_calls_made','new_verified_count','certificate_status')};write(output_dir/f'v2_tal_stage3g_{mode}_summary.json',summary)
 return summary

def validate(*,output_dir:Path,mode:str='run')->dict[str,Any]:
 pre=_preflight(output_dir);run=output_dir/mode;m=obj(run/'v2_tal_stage3g_manifest.json','MANIFEST');audit=obj(run/'v2_tal_stage3g_audit.json','AUDIT')
 ...
 result=rows(run/'v2_tal_spatial_anchor_groundings.jsonl','GROUNDINGS')
 ...
 review=rows(run/'v2_tal_spatial_anchor_review_packet_index.jsonl','REVIEW_PACKET_INDEX')
 ...
 return {'status':'PASS','mode':mode,'stage_status':m['stage_status'],'planned_model_calls':m['planned_model_calls'],'new_model_calls':m['new_model_calls'],'cache_hits':m['cache_hits'],'canonical_grounding_result_sha256':m['canonical_grounding_result_sha256'],'ready_for_spatial_grounding_audit':True,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE'}
