"""ReliVE-v2 Stage 3G-A v3 strict JSON-object anchor grounding."""
...
FORMAT="relive-v2-spatial-anchor-grounding-v3"
...
def _prompt(entry:dict[str,Any])->str:
 ...
 text=files('relive').joinpath('prompts','v2_tal_spatial_anchor_grounding_v3.txt').read_text(encoding='utf-8').format(observation_claim=entry['observation_claim'],observation_role=entry['observation_role'],required_roles=', '.join(required),contextual_roles=', '.join(context) or 'none',role_descriptions='\n'.join(f'- {r}: {ROLES[r]}' for r in all_roles)).rstrip()
 return text

def _parse(raw:str,expected:list[str],width:int,height:int)->tuple[list[dict[str,Any]],str|None]:
 text=raw.strip()
 # v3's generated object contract does not permit Markdown fences or arrays.
...
def preflight(*,stage3f_dir:Path,stage3c_dir:Path,stage3d_dir:Path,stage3e_dir:Path,video_index_dir:Path,config_path:Path,policy_path:Path,output_dir:Path,smoke_selection_manifest:Path|None=None,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
 ...
 if policy.get('format')!='relive-v2-spatial-anchor-grounding-v3-policy-v1':raise SpatialAnchorGroundingError('POLICY_INVALID')
 ...
 if not callable(getattr(backend,'infer_with_generation_metadata',None)):raise SpatialAnchorGroundingError('GENERATION_METADATA_INTERFACE_UNAVAILABLE')
 frozen=[]
 for e in entries:
  frozen.append(e|{'image_path_sha256':hashlib.sha256(e['image_path'].encode()).hexdigest()})
 out={'format':FORMAT,'status':'PASS','mode':'preflight','provider_type':policy['provider_type'],'provider_name':policy['provider_name'],'stage3f_manifest_sha256':upstream['stage3f_manifest_sha256'],'upstream_sha256':upstream,'policy_sha256':sha(policy_path),'config_sha256':sha(config_path),'generation_parameters':effective['backend']['generation'],'generation_contract':{'generation_contract_version':policy['generation_contract_version'],'native_generation_metadata_supported':True,'json_schema_constrained_decoding_supported':False,'enforcement':'VERSIONED_STRENGTHENED_PROMPT_AND_FAIL_CLOSED_PARSER'},'model_fingerprint':fingerprint,'planned_model_calls':len(entries),'unique_task_anchor_count':len(entries),'frozen_calls':frozen,**selection,'model_calls_made':0,'new_model_calls':0,'cache_hits':0,'backend_loaded':True,'cache_opened':False,'frames_read':len(entries),'unique_frame_bytes_opened':len({e['frame_sha256'] for e in entries}),'videos_read':0,'gt_used':False,'assistant_or_gt_values_accessed':False,'boxes_created':0,'points_created':0,'masks_created':0,'masklets_created':0,'support_tubes_created':0,'certificate_created':False,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE'}
 out['preflight_content_sha256']=stable_hash(out);output_dir.mkdir(parents=True);write(output_dir/'stage3g_v3_preflight.json',out);return out

def _preflight(root:Path)->dict[str,Any]:
 p=obj(root/'stage3g_v3_preflight.json','PREFLIGHT')
...
def _cache_key(entry:dict[str,Any],pre:dict[str,Any],policy:dict[str,Any])->str:
 return stable_hash({'format':FORMAT,'provider':{'type':policy['provider_type'],'name':policy['provider_name']},'model_fingerprint':pre['model_fingerprint'],'config_sha256':pre['config_sha256'],'stage3f_manifest_sha256':pre['stage3f_manifest_sha256'],'grounding_task_id':entry['spatial_grounding_task_id'],'anchor_candidate_id':entry['anchor_candidate_id'],'unique_visual_frame_id':entry['unique_visual_frame_id'],'frame_sha256':entry['frame_sha256'],'prompt_sha256':entry['prompt_sha256'],'parser_version':policy['parser_version'],'generation_contract_version':policy['generation_contract_version'],'generation':pre['generation_parameters'],'coordinate_policy':policy['coordinate_conversion_policy_version']})
...
def execute(*,output_dir:Path,config_path:Path,policy_path:Path,mode:str,backend_factory:Callable[[dict[str,Any]],Any]=make_backend)->dict[str,Any]:
 ...
 for entry in entries:
  ...
  try: stored=cache.get_json('stage3g_v3',key)
  ...
  if stored is not None:
   ...
   generated=backend.infer_with_generation_metadata({'prompt':entry['prompt'],'image_paths':[entry['image_path']],'frame_ids':[entry['unique_visual_frame_id']]})
   if not isinstance(generated,dict) or not isinstance(generated.get('raw_response'),str) or not isinstance(generated.get('generation_metadata'),dict):raise SpatialAnchorGroundingError('GENERATION_METADATA_INVALID')
   raw=generated['raw_response'];metadata=generated['generation_metadata']
   ...
   result={'raw_response':raw,'raw_response_sha256':hashlib.sha256(raw.encode()).hexdigest(),'raw_response_closure':_closure(raw),'generation_metadata':metadata,'components':components,'failure_reason':failure,'status':_status(components,entry['required_component_roles'],failure)}
   cache.put_json('stage3g_v3',key,{'cache_key':key,'result':result});new+=1;cache_hit=False
 ...
 for name,value in {'v2_tal_spatial_anchor_groundings_v3.jsonl':results,'v2_tal_spatial_anchor_grounding_v3_unresolved.jsonl':unresolved,'v2_tal_spatial_anchor_grounding_v3_bindings.jsonl':bindings,'v2_tal_spatial_anchor_grounding_v3_trace.jsonl':traces}.items():write(run_dir/name,value)
 review=[_review(run_dir,row) for row in results];write(run_dir/'v2_tal_spatial_anchor_v3_review_packet_index.jsonl',review)
 hashes={path.name:sha(path) for path in run_dir.glob('v2_tal_spatial_anchor_grounding*.jsonl')}|{'v2_tal_spatial_anchor_v3_review_packet_index.jsonl':sha(run_dir/'v2_tal_spatial_anchor_v3_review_packet_index.jsonl')}
 ...
 manifest={'format':FORMAT,'status':'PASS','mode':mode,'stage_status':'SPATIAL_ANCHOR_GROUNDINGS_GENERATED_UNVERIFIED','candidate_status':CANDIDATE,'stage3f_manifest_sha256':pre['stage3f_manifest_sha256'],'upstream_sha256':pre['upstream_sha256'],'preflight_sha256':sha(output_dir/'stage3g_v3_preflight.json'),'provider_type':policy['provider_type'],'provider_name':policy['provider_name'],'model_fingerprint':pre['model_fingerprint'],'policy_sha256':pre['policy_sha256'],'config_sha256':pre['config_sha256'],'generation_contract':pre['generation_contract'],'selection_mode':pre['selection_mode'],'smoke_selection_manifest_sha256':pre['smoke_selection_manifest_sha256'],'planned_model_calls':expected,'new_model_calls':new,'cache_hits':hits,'logical_model_calls':expected,'canonical_grounding_result_sha256':stable_hash(canonical),'artifact_sha256':hashes,'grounding_result_count':len(results),'unresolved_count':len(unresolved),'review_item_count':len(review),'parse_failure_count':parse_failures,'schema_violation_count':schema_failures,'parse_or_schema_compliant_count':expected-parse_failures-schema_failures,'parse_or_schema_compliance_rate':(expected-parse_failures-schema_failures)/expected if expected else 0.0,'strict_required_role_contract_compliant_count':strict_contract,'strict_required_role_contract_compliance_rate':strict_contract/expected if expected else 0.0,'visible_required_role_count':sum(1 for r in results for c in r['components'] if c['role'] in r['required_component_roles'] and c['visibility']=='VISIBLE'),'legal_bbox_count':sum(1 for r in results for c in r['components'] if c['bbox_2d_raw'] is not None),'actual_unlocalized_role_count':sum(1 for r in results for c in r['components'] if c['visibility']!='VISIBLE'),'frames_read':expected,'unique_frame_bytes_opened':len({r['frame_sha256'] for r in results}),'videos_read':0,'model_calls_made':new,'backend_loaded':True,'cache_opened':True,'gt_used':False,'assistant_or_gt_values_accessed':False,'boxes_created':sum(1 for r in results for c in r['components'] if c['bbox_2d_raw'] is not None),'points_created':0,'masks_created':0,'masklets_created':0,'support_tubes_created':0,'certificate_created':False,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE','ready_for_spatial_grounding_audit':bool(results),'ready_for_dense_box_tube_generation':False,'ready_for_sam2_propagation':False,'ready_for_evidence_verification':False}
 manifest['manifest_content_sha256']=stable_hash(manifest);write(run_dir/'v2_tal_stage3g_v3_manifest.json',manifest)
 audit={k:manifest[k] for k in ('format','status','mode','stage3f_manifest_sha256','upstream_sha256','planned_model_calls','new_model_calls','cache_hits','logical_model_calls','frames_read','unique_frame_bytes_opened','videos_read','model_calls_made','backend_loaded','cache_opened','gt_used','assistant_or_gt_values_accessed','boxes_created','points_created','masks_created','masklets_created','support_tubes_created','certificate_created','new_verified_count','certificate_status')}|{'artifact_sha256':{**hashes,'v2_tal_stage3g_v3_manifest.json':sha(run_dir/'v2_tal_stage3g_v3_manifest.json')}}
 audit['audit_content_sha256']=stable_hash(audit);write(run_dir/'v2_tal_stage3g_v3_audit.json',audit)
 summary={k:manifest[k] for k in ('format','status','mode','stage_status','planned_model_calls','new_model_calls','cache_hits','logical_model_calls','canonical_grounding_result_sha256','grounding_result_count','unresolved_count','review_item_count','model_calls_made','new_verified_count','certificate_status')};write(output_dir/f'v2_tal_stage3g_v3_{mode}_summary.json',summary)
 return summary

def validate(*,output_dir:Path,mode:str='run')->dict[str,Any]:
 pre=_preflight(output_dir);run=output_dir/mode;m=obj(run/'v2_tal_stage3g_v3_manifest.json','MANIFEST');audit=obj(run/'v2_tal_stage3g_v3_audit.json','AUDIT')
 ...
 result=rows(run/'v2_tal_spatial_anchor_groundings_v3.jsonl','GROUNDINGS')
 ...
 review=rows(run/'v2_tal_spatial_anchor_v3_review_packet_index.jsonl','REVIEW_PACKET_INDEX')
 ...
 return {'status':'PASS','mode':mode,'stage_status':m['stage_status'],'planned_model_calls':m['planned_model_calls'],'new_model_calls':m['new_model_calls'],'cache_hits':m['cache_hits'],'canonical_grounding_result_sha256':m['canonical_grounding_result_sha256'],'parse_failure_count':m['parse_failure_count'],'schema_violation_count':m['schema_violation_count'],'visible_required_role_count':m['visible_required_role_count'],'legal_bbox_count':m['legal_bbox_count'],'actual_unlocalized_role_count':m['actual_unlocalized_role_count'],'ready_for_spatial_grounding_audit':True,'new_verified_count':0,'certificate_status':'NOT_APPLICABLE'}
